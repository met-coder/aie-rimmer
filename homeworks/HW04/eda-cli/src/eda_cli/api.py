...
async def flags_from_csv(file: UploadFile = File(...)) -> FlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_flags_flags)
    ...
    # Примитивный лог — на семинаре можно обсудить, как это превратить в нормальный logger
    print(
        f"[flags-from-csv] filename={file.filename!r} "
        f"flags={flags_all}"
    )
    
    return FlagsResponse(flags=flags_all)