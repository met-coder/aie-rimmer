...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # НОВАЯ ЭВРИСТИКА 2: has_suspicious_id_duplicates
    flags["has_suspicious_user_id_duplicates"] = False 
    for col in summary.columns:
        if col.name == "user_id":
            flags[f"has_suspicious_user_id_duplicates"] = col.unique < col.non_null
            break
    ...
    if summary.n_cols > 100:
        score -= 0.1
    if flags["has_suspicious_user_id_duplicates"]:
        # Дубликаты user_id нарушают целостность данных — критично
        score -= 0.25
    if flags["has_constant_columns"]:
        # Константные колонки бесполезны для ML/анализа
        score -= 0.05
